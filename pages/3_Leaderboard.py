...
# --- Načti profily (JEDINÝ zdroj pro leaderboard) ---
try:
...
rows.sort(key=lambda x: (-x["points"], x["email"]))
...
if is_admin:
    with st.expander("🔍 Debug (profiles.points)"):
        st.caption("Tady jsou přímo hodnoty uložené v profiles.points (to je zdroj leaderboardu).")
        st.dataframe(
            [{"email": r["email"], "points": r["points"], "is_admin": r["is_admin"]} for r in rows],
